...
<div class="card ">
  ...
  <div class="card-body">
    ...
    @endif
     <form class="row g-3" method="POST" action="{{ url('update-data/'.$abonnes->id)}}">
    @csrf
    @method('PUT')

        <div class="col-4">
           <label for="inputName" class="form-label">Nom(s)</label>
           <input required type="text" name="nom" value="{{$abonnes->nom}}" class="form-control" id="inputName">
        </div>
        <div class="col-4">
            <label for="inputFirstname" class="form-label">Prénom(s)</label>
            <input required type="text" name="prenom" value="{{$abonnes->prenom}}"class="form-control" id="inputFirstname">
        </div>
        <div class="col-4">
                  <label for="inputDatenaissance" class="form-label">Date de naissance</label>
                  <input required type="date" name="date_naissance" value="{{$abonnes->date_naissance}}"class="form-control" id="inputDatenaissance">
        </div>
        <div class="col-4">
                  <label for="inputDatenaissance" class="form-label">Niveau scolaire</label>
                  <select class="form-control" name="niveau_scolaire" value="{{$abonnes->niveau_scolaire}}">
                ...
                <div class="col-4">
                  ...
                  <input required type="text" name="adresse" value="{{$abonnes->adresse}}"class="form-control" id="inputAdresseHabitation">
                </div>
                 
                <div class="col-4">
                  <label for="inputDatenaissance" class="form-label">Telephone</label>
                  <input required type="text" value="{{$abonnes->telephone}}" name="telephone" class="form-control" id="inputTelephone">
                </div>

                 <div class="col-4">
                  <label for="inputDatenaissance" class="form-label">Categorie Abonnement</label>
                  <select class="form-control" name="categorie_abonnement" value="{{$abonnes->categorie_abonnement}}">
                ...
                <div class="col-4">
                  ...
                  <input required type="integer"  name="montant_paye" value="{{$abonnes->montant_paye}}" class="form-control" id="inputPrimeSouscription">
                </div>                                
                <div class="col-12">
                    <button type="submit" class="btn btn-primary "><i class="fas fa-save"></i> Valider la modification</button>
